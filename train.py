...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    if rank in [-1, 0]:
        ...
        if not opt.resume:
            ...
            c = torch.tensor(
                [int(x) for x in labels[:, 0]], dtype=torch.long
            )  # classes
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", type=str, default="yolov5m", help="yolo model to use"
    )
    parser.add_argument(
        "--classes", type=str, default="", required=False, help="model.yaml path"
    )
    parser.add_argument(
        "--dataset-path", type=str, required=True, help="path to dataset folder"
    )
    parser.add_argument(
    ...
    if opt.resume:  # resume an interrupted run
        ...
        if not isinstance(opt.data, dict):
