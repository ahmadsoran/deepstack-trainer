    ...
    def _initialize_biases(
        ...
        for mi, s in zip(m.m, m.stride):  # from
            ...
            bias_offset = torch.tensor(
                math.log(8 / (640 / s) ** 2), dtype=b.dtype, device=b.device
            )
            b.data[:, 4] += bias_offset  # obj (8 objects per 640 image)
            if cf is None:
                cls_offset = torch.tensor(
                    math.log(0.6 / (m.nc - 0.99)), dtype=b.dtype, device=b.device
                )
                b.data[:, 5:] += cls_offset  # cls
            else:
                b.data[:, 5:] += torch.log(cf / cf.sum())  # cls
    ...
    for i, (f, n, m, args) in enumerate(
        ...
        for j, a in enumerate(args):
            try:
                ...
                pass  # keep as string if cannot eval
